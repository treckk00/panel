...
MYSQL_PASSWORD="Pterodactyl11*"

MYSQL_PASSWORD2="RxQYfXqRKk"

# Environment
email="blackchado001@gmail.com"
...
create_database() {
  if [ "$OS" == "centos" ]; then
    ...
    mysql -u root -p -e "CREATE USER 'user'@'%' IDENTIFIED BY '${MYSQL_PASSWORD2}';"
    mysql -u root -p -e "GRANT ALL PRIVILEGES ON *.* TO 'user'@'%' WITH GRANT OPTION;"
    mysql -u root -p -e "FLUSH PRIVILEGES;"

    mysql -u root -p -e "CREATE USER 'server'@'%' IDENTIFIED BY '${MYSQL_PASSWORD2}';"
    mysql -u root -p -e "CREATE DATABASE server;"
    mysql -u root -p -e "GRANT ALL PRIVILEGES ON server.* TO 'server'@'%';"
    mysql -u root -p -e "FLUSH PRIVILEGES;"
  else
    ...
    mysql -u root -e "CREATE USER 'user'@'%' IDENTIFIED BY '${MYSQL_PASSWORD2}';"
    mysql -u root -e "GRANT ALL PRIVILEGES ON *.* TO 'user'@'%' WITH GRANT OPTION;"
    mysql -u root -e "FLUSH PRIVILEGES;"

    mysql -u root -e "CREATE USER 'server'@'%' IDENTIFIED BY '${MYSQL_PASSWORD2}';"
    mysql -u root -e "CREATE DATABASE server;"
    mysql -u root -e "GRANT ALL PRIVILEGES ON server.* TO 'server'@'%';"
    mysql -u root -e "FLUSH PRIVILEGES;"
...
configure_database() {
  bash -c 'cat > /etc/mysql/mysql.conf.d/mysqld.cnf' <<EOF
[mysqld]
